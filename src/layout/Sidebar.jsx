...
export default function Sidebar() {
    ...
    return isSidebarOpen ? (
        <div
            data-testid="sidebar"
            className="hidden md:block col-span-4 md:col-span-2 h-screen py-3 overflow-y-auto scroll scrollbar-hide"
        >
            {sidebarData.map(({ icon, text: textContent, route, tab }) => (
                <Link
                    ...
                    onClick={() =>
                        handleSectionSelect(
                            textContent === "Home" ? "All" : textContent
                        )
                    }
                    className={`${
            ...
            {sidebarData.map(({ text: textContent, icon, route, tab }) => {
                return (
                    ...
                    >
                        ...
                        <p className="text-[10px] mt-2 break-words">
                            {textContent}
                        </p>
                    </Link>
                );
